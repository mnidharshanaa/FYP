...
| 4 (remainder) | MAD variants for Table II (mad_sparse_half, mad_random) — Standard MAD fully-connected already covered by Module 2 | ✅ done, 300 tests passing (`src/baselines/mad_variants.py`, `src/baselines/mad_variant_orchestration.py`, `scripts/07_run_mad_variants.py`) |
| Final | **Propagation Control Module** — the proposed novel contribution: DIGRA's utility (U) + Base-2-style per-agent entailment-clustering reliability (R) combined into PS=U^λ·R^(1-λ), propagate/verify/suppress decisions, four-quadrant classification, conditional RAG (real retrieval, snapshot-based for reproducibility — never oracle/self-verification), full Level 1-4 logging, λ-ablation/four-quadrant/RAG-effect report. | ✅ core + report layer built and wired end-to-end, 423 tests passing. `src/reliability/{propagation_score,information_flow,schema,entailment,propagation_debate,orchestration}.py`, `src/rag/{retriever,verification}.py`, `src/reports/propagation_report.py`, `scripts/09_build_evidence_snapshot.py` (one-time, needs internet), `scripts/10_run_propagation_control.py` (the runnable experiment), `scripts/11_generate_propagation_report.py` (CPU-only report). **NOT YET built:** the N=10 pilot hasn't been RUN (only the code path is tested against FakeLLMClient); PS-validity binning (spec Section 23); the calibration/test-split enforcement (spec Section 7) — right now nothing stops you from tuning lambda on the same data you evaluate it on, that discipline is on you until this is built. |
