"""
Unified DIGRA debate loop.

Runs one of three variants, controlled by `variant`:
  "digra"             — entropy-based IGR only, exactly the base paper's method.
  "digra_rag"         — + evidence-adjusted entropy (src/rag/evidence.py).
  "digra_rag_memory"  — + persistent trust weighting (src/memory/trust.py).

All three share this exact same loop — the variant only controls which
corrections are active when computing the entropy fed into IGR. This is
deliberate: it's what makes the three conditions a genuine, fair
ablation (same code path, same seeds, same everything else) rather than
three separately-written, potentially-inconsistent implementations.
...
communication logic differs (dynamic IGR-based selection instead of
fully-connected).
"""
...
def run_digra_debate(
    ...
    for i in range(n_agents):
        ...
        agent_records[i].append(AgentRoundRecord(
            ...
            partners_selected=None, igr_score=None,
        ))
    ...
    for round_idx in range(2, n_rounds + 1):
        ...
        for i in range(n_agents):
            if frozen[i] and i not in agents_to_generate:
                ...
                carried = _dc.replace(last, round_idx=round_idx, partners_selected=None, igr_score=None)
