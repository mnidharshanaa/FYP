...
Runs DIG/DIGRA experiments (one of: dig, digra, digra_rag, digra_rag_memory)
across every dataset x agent-count x hallucination-setup x seed combination
in the config, seeded from scripts/01_build_pools.py's pools. Resumable.

"dig" is the IG-only ablation (no entropy normalization, no RAG, no
memory) — start here, since it's the simplest of the four and the
natural next module after Standard MAD. It's run through this same
script/orchestration path as digra/digra_rag/digra_rag_memory (see
src/digra/digra_debate.py's module docstring for why that's deliberate).
...
Usage:
    ...
    # then the real runs, one per variant (dig first, then digra, then the
    # RAG/memory extensions — each builds on the previous conceptually,
    # though they don't depend on each other's output files):
