...
VARIANT_ORDER = ["dig", "digra", "digra_rag", "digra_rag_memory"]
VARIANT_LABELS = {
    "dig": "DIG (IG-only)", "digra": "DIGRA", "digra_rag": "DIGRA+RAG",
    "digra_rag_memory": "DIGRA+RAG+Memory",
}
FILENAME_RE = re.compile(r"^(?P<dataset>[a-z]+)_(?P<model>[a-z]+)_(?P<variant>digra(?:_rag)?(?:_memory)?|dig)_agents(?P<n_agents>\d+)\.jsonl$")
# Variants whose agent_records carry partners_selected/igr_score, i.e.
# everything with a dynamic (non-fully-connected) communication topology.
DYNAMIC_TOPOLOGY_VARIANTS = ["dig", "digra", "digra_rag", "digra_rag_memory"]
...
def main(results_dir: str, out_dir: str, baseline_dir: str = None) -> None:
    ...
    for model in models:
        for dataset in datasets:
            ...
            for variant in VARIANT_ORDER:
                ...
                comm = compute_communication_stats(results)
                ...
                if comm["sparsity"] is not None:
                    line += f"  sparsity={comm['sparsity']:.2f} ({comm['mean_selected_edges']:.1f}/{comm['candidate_edges']} edges)"
                    line += f"  mean_score={comm['mean_selection_score']:.3f}"
                ...
                flow = compute_information_flow(results)
                if flow["total"]:
                    print(
                        f"    info flow (n={flow['total']}): "
                        f"correct->correct={flow['correct_to_correct_ratio']:.2f}  "
                        f"correct->wrong={flow['correct_to_wrong_ratio']:.2f}  "
                        f"wrong->correct={flow['wrong_to_correct_ratio']:.2f}  "
                        f"wrong->wrong={flow['wrong_to_wrong_ratio']:.2f}"
                    )
    ...
    _plot_propagation_rates(data, datasets, models, out_dir_path)
    _plot_cost_comparison(data, datasets, models, out_dir_path)
    _plot_communication_sparsity(data, datasets, models, out_dir_path)
    _plot_information_flow(data, datasets, models, out_dir_path)
...
def _plot_propagation_rates(data, datasets, models, out_dir):
    """MR (Misleading Rate) and CR (Correction Rate) per round, one line
    per available variant — base paper's Table I quantities plotted as
    trajectories rather than a static table, so propagation dynamics
    (not just endpoint accuracy) are visible for every module."""
    for model in models:
        for dataset in datasets:
            variants = _available_variants(data, dataset, model)
            if not variants:
            ...
            fig, (ax_mr, ax_cr) = plt.subplots(1, 2, figsize=(11, 4))
            for v in variants:
                metrics = compute_propagation_metrics(data[(dataset, model, v)])
                rounds = sorted(r for r in metrics["MR"] if metrics["MR"][r] is not None)
                ax_mr.plot(rounds, [metrics["MR"][r] for r in rounds], marker="o", label=VARIANT_LABELS[v])
                rounds_cr = sorted(r for r in metrics["CR"] if metrics["CR"][r] is not None)
                ax_cr.plot(rounds_cr, [metrics["CR"][r] for r in rounds_cr], marker="o", label=VARIANT_LABELS[v])

            ax_mr.set_xlabel("Debate round")
            ax_mr.set_ylabel("Misleading Rate (MR)")
            ax_mr.set_title(f"Misleading rate — {dataset} / {model}")
            ax_mr.set_ylim(0, 1)
            ax_mr.legend(fontsize=8)

            ax_cr.set_xlabel("Debate round")
            ax_cr.set_ylabel("Correction Rate (CR)")
            ax_cr.set_title(f"Correction rate — {dataset} / {model}")
            ax_cr.set_ylim(0, 1)
            ax_cr.legend(fontsize=8)

            plt.tight_layout()
            fname = out_dir / f"propagation_rates_{dataset}_{model}.png"
            plt.savefig(fname, dpi=150)
            plt.close(fig)
            print(f"  wrote {fname}")


def _plot_communication_sparsity(data, datasets, models, out_dir):
    """Base paper's Table III: fraction of candidate edges actually
    selected, per variant. Only variants with dynamic topology have this
    (Standard MAD is always fully connected, sparsity=1.0 by definition,
    and isn't plotted here since it has no partners_selected field)."""
    for model in models:
        for dataset in datasets:
            variants = [v for v in DYNAMIC_TOPOLOGY_VARIANTS if (dataset, model, v) in data]
            if not variants:
                continue
            sparsities = [compute_communication_stats(data[(dataset, model, v)])["sparsity"] for v in variants]
            labels = [VARIANT_LABELS[v] for v in variants]

            fig, ax = plt.subplots(figsize=(6, 4))
            bars = ax.bar(labels, sparsities, color="#64B5CD")
            ax.axhline(1.0, color="#999999", linestyle="--", linewidth=1, label="Fully connected (MAD)")
            ax.set_ylabel("Sparsity (selected / candidate edges)")
            ax.set_ylim(0, 1.1)
            ax.set_title(f"Communication sparsity — {dataset} / {model}")
            for bar, s in zip(bars, sparsities):
                if s is not None:
                    ax.text(bar.get_x() + bar.get_width() / 2, s + 0.02, f"{s:.2f}", ha="center")
            ax.legend(fontsize=8)
            plt.tight_layout()
            fname = out_dir / f"sparsity_{dataset}_{model}.png"
            plt.savefig(fname, dpi=150)
            plt.close(fig)
            print(f"  wrote {fname}")


def _plot_information_flow(data, datasets, models, out_dir):
    """Base paper's Fig. 6(a): stacked bar of correct->correct,
    correct->wrong, wrong->correct, wrong->wrong transition ratios per
    variant — shows whether a variant lets accurate agents get misled
    (correct->wrong, the bad case) vs. corrects hallucinating agents
    (wrong->correct, the good case)."""
    for model in models:
        for dataset in datasets:
            variants = [v for v in DYNAMIC_TOPOLOGY_VARIANTS if (dataset, model, v) in data]
            if not variants:
                continue
            flows = [compute_information_flow(data[(dataset, model, v)]) for v in variants]
            if not any(f["total"] for f in flows):
                continue
            labels = [VARIANT_LABELS[v] for v in variants]

            cc = [f["correct_to_correct_ratio"] or 0 for f in flows]
            cw = [f["correct_to_wrong_ratio"] or 0 for f in flows]
            wc = [f["wrong_to_correct_ratio"] or 0 for f in flows]
            ww = [f["wrong_to_wrong_ratio"] or 0 for f in flows]

            fig, ax = plt.subplots(figsize=(7, 4))
            x = range(len(labels))
            bottom = [0] * len(labels)
            for vals, label, color in [
                (cc, "correct->correct", "#55A868"), (wc, "wrong->correct", "#4C72B0"),
                (cw, "correct->wrong", "#C44E52"), (ww, "wrong->wrong", "#999999"),
            ]:
                ax.bar(x, vals, bottom=bottom, label=label, color=color)
                bottom = [b + v for b, v in zip(bottom, vals)]
            ax.set_xticks(list(x))
            ax.set_xticklabels(labels)
            ax.set_ylabel("Share of round-to-round transitions")
            ax.set_title(f"Information flow — {dataset} / {model}")
            ax.legend(fontsize=8, loc="upper center", bbox_to_anchor=(0.5, -0.15), ncol=2)
            plt.tight_layout()
            fname = out_dir / f"info_flow_{dataset}_{model}.png"
            plt.savefig(fname, dpi=150)
            plt.close(fig)
            print(f"  wrote {fname}")
