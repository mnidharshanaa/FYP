...
def compute_token_savings(baseline_results: list, comparison_results: list) -> dict:
    """
    Checklist's "Token savings" (Module 4/DIGRA, base paper's Table III):
    fractional reduction in mean total-tokens-per-debate of
    `comparison_results` (e.g. dig/digra/digra_rag/digra_rag_memory)
    relative to `baseline_results` (Standard MAD's fully-connected
    topology, since that's what the base paper's Table III measures
    savings against). Requires both result sets' debates to carry
    `total_tokens` — Standard MAD's output schema must include that
    field the same way DigraDebateResult does for this to be
    meaningful; if the baseline set predates that field, this returns
    None rather than silently comparing against zero.
    """
    baseline_tokens = compute_token_stats(baseline_results)["mean_total_tokens_per_debate"]
    comparison_tokens = compute_token_stats(comparison_results)["mean_total_tokens_per_debate"]

    if not baseline_tokens or comparison_tokens is None:
        return {"savings_fraction": None, "baseline_mean_tokens": baseline_tokens, "comparison_mean_tokens": comparison_tokens}

    return {
        "savings_fraction": 1.0 - (comparison_tokens / baseline_tokens),
        "baseline_mean_tokens": baseline_tokens,
        "comparison_mean_tokens": comparison_tokens,
    }
