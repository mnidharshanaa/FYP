...
def test_token_savings_basic():
    baseline = [_debate([[_agent_record(1, True)]])]
    baseline[0]["total_tokens"] = 100
    comparison = [_debate([[_agent_record(1, True)]])]
    comparison[0]["total_tokens"] = 60
    savings = compute_token_savings(baseline, comparison)
    assert savings["savings_fraction"] == pytest.approx(0.4)
    assert savings["baseline_mean_tokens"] == 100
    assert savings["comparison_mean_tokens"] == 60


def test_token_savings_none_when_baseline_missing():
    savings = compute_token_savings([], [_debate([[_agent_record(1, True)]])])
    assert savings["savings_fraction"] is None
